  ...
  public PaginationResult getAll(String sort, int pageNumber, int maxResults) {
    ...
    int totalPages =
        (int)
            Math.ceil(
                (double)
                        tagRepository.getMaxAvailableResultsOfGetListBySpecification(
                            new GetAllTagsSpecification(sort))
                    / maxResults);
    return new PaginationResult(tagModelMapper.listToDto(tags), totalPages);
  }
  ...
  private void validate(TagDto tagDto) {
    ...
    if (tagDto.getName() != null) {
      if (tagDto.getName().length() < MIN_NAME_LENGTH
        ...
        exceptionMessages.add("Tag name must be between " + MIN_NAME_LENGTH + " and " + MAX_NAME_LENGTH + " characters.");
      }
    }
