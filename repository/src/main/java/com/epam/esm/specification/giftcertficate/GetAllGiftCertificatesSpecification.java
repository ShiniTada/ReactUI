...
public class GetAllGiftCertificatesSpecification implements Specification<GiftCertificate> {
  ...
  public CriteriaQuery<GiftCertificate> getCriteriaQuery(CriteriaBuilder builder) {
    ...
    if (sort != null) {
      if(sort.equals("creation_date")) {
        criteria.orderBy(builder.asc(root.get("creationDate")));
      }
      if(sort.equals("last_modified")) {
        criteria.orderBy(builder.asc(root.get("lastModified")));
      }
